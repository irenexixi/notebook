...
const checkRespStatus = resp => {
    ...
    return new Promise((resolve, reject) => {
        if (resp && data.code === 200) {
            data.remark &&
        Message({ message: data.remark, type: 'success', showClose: false });
      ...
      // 页面超时，重新刷新页面
            ...
            let COMMON_LOGIN_ROUTER = '';
            window.location.href = `${COMMON_LOGIN_ROUTER}?next=${encodeURIComponent(
...
const request = (
  ...
  jsonType = true
    ...
    let sessionId =
    localStorage.getItem('sessionId') || '1f01949454ab9284738a2a7a5b6839be';
    var options = {
        headers: {
        ...
        url + sessionId + (params ? JSON.stringify(params) : '')
      )
        },
        method: method,
        url: '',
